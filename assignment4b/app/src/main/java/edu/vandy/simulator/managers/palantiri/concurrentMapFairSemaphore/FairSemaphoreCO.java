...
public class FairSemaphoreCO
     ...
     */
    // TODO -- you fill in here.
    private LinkedList<Waiter> mWaiterQueue;
    ...
    protected boolean tryToGetPermitUnlocked() {
        ...
        // can be obtained.
        if (mWaiterQueue.isEmpty() && mAvailablePermits > 0) {
            mAvailablePermits--;
            return true;
        }
        return false;
    ...
    protected void waitForPermit() throws InterruptedException {
        ...
        try {
            ...
            try {
                mLock.lock();
                released = mWaiterQueue.remove(waiter);
                if (!released)
                    release();
            } finally {
                mLock.unlock();
